    ...
    <div className="min-h-screen bg-gradient-to-br from-cream-50 via-white to-cream-100">
      ...
      <main className="pt-32 pb-20">
        <div className="container mx-auto px-4">
            ...
            {avantages.map((a, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 shadow-lg text-center hover:shadow-xl transition">
                <div className="w-14 h-14 bg-cream-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <a.icon className="text-terra-600 text-2xl" />
                </div>
                <h3 className="font-bold text-brown-800 mb-2">{a.titre}</h3>
          ...
          {/* Processus */}
          <div className="mb-20">
            <h2 className="text-3xl font-playfair font-bold text-brown-800 text-center mb-12">
              ...
              {etapes.map((e, i) => (
                <div key={i} className="relative">
                  <div className="bg-white rounded-2xl p-6 shadow-lg h-full">
                    <div className="text-5xl font-playfair font-bold text-cream-100 mb-3 leading-none">{e.num}</div>
                    <h3 className="font-bold text-brown-800 text-lg mb-2">{e.titre}</h3>
                    <p className="text-sm text-gray-500 leading-relaxed">{e.desc}</p>
                  </div>
                  {i < etapes.length - 1 && (
                    <div className="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 z-10 w-6 h-6 bg-terra-500 rounded-full items-center justify-center text-white text-xs">
                      <FaArrowRight />
                    </div>
                  )}
                </div>
          ...
          <div className="bg-brown-800 rounded-2xl p-8 md:p-12 mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <FaCheckCircle className="text-terra-400 mt-0.5 flex-shrink-0" />
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
              <h2 className="text-2xl font-playfair font-bold text-brown-800 mb-2 text-center">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Nom complet *</label>
                    <input type="text" name="nom" value={formData.nom} onChange={handleChange} required
                      className="w-full px-4 py-3 border border-cream-200 rounded-xl focus:border-terra-500 focus:ring-2 focus:ring-terra-500/20 outline-none text-sm" />
                  </div>
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Email *</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Téléphone</label>
                    <input type="tel" name="telephone" value={formData.telephone} onChange={handleChange}
                      placeholder="(+237) ..."
                      className="w-full px-4 py-3 border border-cream-200 rounded-xl focus:border-terra-500 focus:ring-2 focus:ring-terra-500/20 outline-none text-sm" />
                  </div>
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Genre littéraire *</label>
                    <select name="genre" value={formData.genre} onChange={handleChange} required
                      className="w-full px-4 py-3 border border-cream-200 rounded-xl focus:border-terra-500 focus:ring-2 focus:ring-terra-500/20 outline-none text-sm bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Titre de l'ouvrage *</label>
                    <input type="text" name="titre" value={formData.titre} onChange={handleChange} required
                      className="w-full px-4 py-3 border border-cream-200 rounded-xl focus:border-terra-500 focus:ring-2 focus:ring-terra-500/20 outline-none text-sm" />
                  </div>
                  <div>
                    <label className="block text-terra-700 text-sm font-medium mb-2">Nombre de pages estimé</label>
                ...
                <div>
                  <label className="block text-terra-700 text-sm font-medium mb-2">Résumé / Synopsis *</label>
                  <textarea name="resume" value={formData.resume} onChange={handleChange} required rows="4"
                    ...
                    className="w-full px-4 py-3 border border-cream-200 rounded-xl focus:border-terra-500 focus:ring-2 focus:ring-terra-500/20 outline-none text-sm resize-none" />
                </div>

                <div>
                  <label className="block text-terra-700 text-sm font-medium mb-2">Message complémentaire</label>
                ...
                <button type="submit" disabled={loading}
                  className="w-full bg-gradient-to-r from-terra-600 to-terra-700 text-white py-4 rounded-xl font-semibold hover:shadow-lg transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2">
                  {loading ? (
            ...
            {/* Contact direct */}
            <div className="mt-8 bg-cream-50 border border-cream-200 rounded-2xl p-6 text-center">
              <p className="text-terra-700 font-medium mb-3">Préférez-vous nous contacter directement ?</p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="mailto:contact@sagesseafricaine.org"
                  className="flex items-center justify-center gap-2 text-terra-600 hover:text-brown-800 transition text-sm">
                  <FaEnvelope /> contact@sagesseafricaine.org
                </a>
