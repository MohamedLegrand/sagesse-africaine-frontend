...
const ParametresPage = () => {
  ...
  const Toggle = ({ value, onChange }) => (
    <div
      onClick={onChange}
      className={`relative w-12 h-6 rounded-full transition-colors duration-300 cursor-pointer ${value ? 'bg-terra-600' : 'bg-gray-300'}`}
    >
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto max-w-4xl">
        <div className="mb-6">
          <h1 className="text-2xl font-playfair font-bold text-brown-800">{t('parametres.titre')}</h1>
          <p className="text-terra-500 text-sm">{t('parametres.sousTitre')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              <FaBell className="text-terra-600 text-2xl" />
              <h2 className="text-xl font-playfair font-bold text-brown-800">{t('parametres.notifications.titre')}</h2>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              <FaPalette className="text-terra-600 text-2xl" />
              <h2 className="text-xl font-playfair font-bold text-brown-800">{t('parametres.apparence.titre')}</h2>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                >
                  <theme.icon className={`text-xl ${settings.apparence.theme === theme.value ? 'text-terra-600' : 'text-gray-400'}`} />
                  <span className={settings.apparence.theme === theme.value ? 'text-terra-700 font-medium' : 'text-gray-600'}>
                    {theme.label}
                  </span>
                </button>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            <div className="flex items-center gap-3 mb-6">
              <FaLanguage className="text-terra-600 text-2xl" />
              <h2 className="text-xl font-playfair font-bold text-brown-800">{t('parametres.langue.titre')}</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <label key={lang.value} className="flex items-center gap-3 p-3 rounded-xl border border-cream-200 cursor-pointer hover:bg-cream-50 transition">
                  <input
                    type="radio"
                    name="langue"
                    value={lang.value}
                  ...
                  />
                  <FaGlobe className="text-terra-500" />
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={updating}
              className="bg-gradient-to-r from-terra-600 to-terra-700 text-white px-8 py-3 rounded-xl font-semibold hover:shadow-lg transition flex items-center gap-2 disabled:opacity-50"
            >
              {updating ? (
